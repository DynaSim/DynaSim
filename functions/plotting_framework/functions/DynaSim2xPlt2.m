...
function [xp] = DynaSim2xPlt2 (data)
    ...
    for i = 1:num_sims
        for k = 1:num_alllabels
            ...
            for j = 1:num_varied
                if isnumeric(data(1).(varied{j})); xp_axes{j}(z) = params{j}(i);
                else xp_axes{j}{z} = params{j}{i};
                end
            end
            
            
            xp_axes{num_varied+1}{z} = pops(k);
            
            j=j+1;
            xp_axes{num_varied+2}{z} = vars(k);
            
        end
    end
    
    axis_names = varied;
    axis_names{num_varied+1} = 'populations';
    axis_names{num_varied+2} = 'variables';
