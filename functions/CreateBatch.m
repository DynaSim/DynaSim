...
% write to file
if strcmp(options.qsub_mode, 'array') % TODO remove temp method
%   fprintf(fScript,'#!/bin/csh\n');
%   fprintf(fScript,'set TASK_ID_START = %i\n',1);
...
%         cmd=sprintf('%s/qmatjobs_memlimit_array %s %s %s',dsFnPath, batch_dir_name, options.memory_limit, dsFnPath); %using csh intermediate
%         cmd = sprintf('qsub -V -hard -l ''h_vmem=%s'' -wd %s -N %s_sim_job -t 1-%i -- %s/qmatjob_array %s sim_job',...
%           options.memory_limit, batch_dir, batch_dir_name, length(jobs), dsFnPath, batch_dir_name); %no pipe
