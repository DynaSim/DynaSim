...
if ~query_flag
  files = lscell(fullfile(ds.getConfig('ds_testData_path'), 'autogenDirs'));
else
  % TODO: fix this
  files = lscell(fullfile(ds.getConfig('ds_testData_path'), 'autogenDirs', ['*' evalin('base','query') '*_autogen_*']));
end
