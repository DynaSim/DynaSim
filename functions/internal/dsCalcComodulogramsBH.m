function data = dsCalcComodulogramsBH(data, varargin)
...
options=dsCheckOptions(varargin,{...
  ...
  'phase_filter_freqs_bh', [0.5 2.0],[],...% Hz, Frequencies bounds for the FILTER for measuring the phase of the "slow" or modulating signal for comodulogramsBH
  'phase_filter_order_bh', 2, [], ... % order of the Butterworth filter for the modulating signal
  'ampl_filter_freqs_bh', [8 14],[],...% Hz, Frequency bounds for the FILTER for measuring the amplitude of the "fast" or carrier signal for comodulogramsBH
  'ampl_filter_order_bh', 2, [], ... % order of the Butterworth filter for the amplitude signal
...
if numel(data)>1
  ...
  data=dsAnalyzeStudy(data,@dsCalcComodulogramsBH,varargin{:});
  return;
end
...
for v=1:length(options.variable)
  ...
  [filt_ph_sig, filt_amp_sig] = filt_signals_butter(X, X, Fs,...
    options.phase_filter_freqs_bh, options.ampl_filter_freqs_bh,...
    options.phase_filter_order_bh, options.ampl_filter_order_bh);
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  [pacmat, pac_angles, comodulogramsBH, modulation_indices] = find_pac_nofilt_bh(...
    filt_ph_sig, filt_amp_sig, Fs, options.calc_comodulogramsBH);
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  data.([var '_ComodulogramsBH_MUA' options.output_suffix]).amplitudes=pacmat;
  data.([var '_ComodulogramsBH_MUA' options.output_suffix]).angles=pac_angles;
  data.([var '_ComodulogramsBH_MUA' options.output_suffix]).comodulogramsBH=comodulogramsBH;
  % data.([var '_ComodulogramsBH_MUA' options.output_suffix]).ampl_freq_axis=freqvec_amp;
  % data.([var '_ComodulogramsBH_MUA' options.output_suffix]).ph_freq_axis=freqvec_ph;
  data.([var '_ComodulogramsBH_MUA' options.output_suffix]).mean_ph_freq=...
    mean(options.phase_filter_freqs_bh);
  data.([var '_ComodulogramsBH_MUA' options.output_suffix]).mean_amp_freq=...
    mean(options.ampl_filter_freqs_bh);

  data.([var '_ComodulogramsBH_MUA' options.output_suffix]).filtered_ph_sig=filt_ph_sig;
  data.([var '_ComodulogramsBH_MUA' options.output_suffix]).filtered_amp_sig=filt_amp_sig;

  if ~ismember([var '_ComodulogramsBH_MUA' options.output_suffix],data.results)
    data.results{end+1}=[var '_ComodulogramsBH_MUA' options.output_suffix];
  end
