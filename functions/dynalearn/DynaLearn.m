classdef DynaLearn < matlab.mixin.SetGet
    ...
    methods
        ...
        function dlPlotAllPotentials(obj, mode, opts)
            ...
            for k = 1:m
                ...
                elseif strcmpi(mode, 'lfp')
                    ...
                    figure();%'Position'%, [0, 0, 1700, min(1700, 1000*(min(k*q, n-1) - (k-1)*q)/(q))]);
        ...
        function dlCalculateError(obj, dlTargetParams)
            ...
            for i = 1:n
                ...
                elseif strcmpi(dlErrorType, 'KLD')
                    ...
                    TempError = dlPowerSpectrumKLD(obj, argsKLD).^2;
                    fprintf(" D[KL] = %f ", TempError);

                elseif strcmpi(dlErrorType, 'LogCor')
        ...
        function dlRunSimulation(obj, dlVaryList, dlOutputParameters)
            ...
            if exist("dlOutputParameters", "var")
        ...
        function dlTrain(obj, dlInputParameters, dlOutputParameters, dlTargetParameters, dlTrainOptions) 
            ...
            try
                ...
                lab = fieldnames(p.p);
                dlTrainOptions('dlTrainIncludeList') = lab(contains(lab, ""));
        ...
        function dlTrainStep(obj, dlLearningRule, dlLambda, dlTrainOptions)
            ...
            try
                ...
                disp("----->Momentum = " + num2str(dlEnhancedMomentum));
            ...
            if localFlag
                ...
                for iLocalParams = 1:numel(Local_LR_Params) % e.g., {E->E, I->E}
                    ...
                    % Indices to V
                    i_Vpre = strcmpi(string(obj.dlVariables), obj.dlGraph.vertices(i_pre).name + "_" + voltage);
                    i_Vpost = strcmpi(string(obj.dlVariables), obj.dlGraph.vertices(i_post).name + "_" + voltage);
                    ...
                    if isempty(find(strcmpi(string(obj.dlWeightsVariables), lkey), 1))
...
%                             rho_ACh = LocalParams.rho_ACh;
                            ...
                            i_eta_ACh = strcmpi(string(obj.dlVariables), obj.dlGraph.vertices(i_post).name + "_" + eta_ACh_name);
...
%                             rho_NE = LocalParams.rho_NE;
                            ...
                            i_eta_NE = strcmpi(string(obj.dlVariables), obj.dlGraph.vertices(i_post).name + "_" + eta_NE_name);
                        ...
                        elseif strcmpi(learningRule, 'Inhibitory')
                            ...
                            i_iE = strcmpi(string(obj.dlVariables), [obj.dlGraph.vertices(i_post).name, '_', iE_name]);
                            i_iI = strcmpi(string(obj.dlVariables), [obj.dlGraph.vertices(i_post).name, '_', iI_name]);
                            ...
                            disp('unknown local plasticity rule');
                        end
                    end
                    % Update weights
                    w = w + local_delta;
            ...
            elseif strcmpi(dlLearningRule, 'BioDeltaRule')
                ...
                for i = l'
                    ...
                    try
                        ...
                        restrictedBnds = dlTrainOptions('dlTrainRestrictBound');
                    ...
                    if ~excludeList(i)
                        ...
                        if restrictedList(i)
                            ...
                            disp(wn);
                            disp(restrictedBnds{restrictedCoefCnt});
            ...
            elseif strcmpi(dlLearningRule, 'GeneralizedStochasticDeltaRule')
                ...
                try
                    ...
                    try
                        ...
                        fprintf("\n---->No boundary values determined. Ignore if you do not need to restrict boundary of variables.\n")
                        restrictedBnds = cell(1, max(size(restrictedCoef)));
                        
                        for rbi = 1:max(size(restrictedCoef))

                            restrictedBnds{rbi} = [-1e+9 1e+9];
                ...
                for i = lg'
                    ...
                    if ~excludeList(i)
                        ...
                        if restrictedList(i)
                            ...
                            try
                                ...
                                if dlTrainOptions('dlUpdateVerbose')
                                    ...
                                    fprintf("\n---->No boundary values/Bad format (ignore this warning if you dont want it)\n");
                        ...
                        elseif dlTrainOptions('dlUpdateVerbose') && restrictedList(i)
                            ...
                            fprintf("-----> Updated: %s : %f (Rstrct. C = %f, LB = %f, UB = %f) \n", lab{i, 1}, mean(mean(val{i, 1})), restrictedCoef{r_}, restrictedBnds{r_}(1), restrictedBnds{r_}(2));
...
%               IndexInOutputs = find(strcmpi(string(obj.dlVariables), obj.dlGraph.vertices(i).name + "_V"));
        ...
        function dlSetSignals(obj)
            ...
            for i = 2:n
                ...
                k = size(dlPotentials{1, i}, 2);
                x(m+1:m+k, :) = dlPotentials{1, i}';
                y(m+1:m+k) = i-1;
                m = m + k;
        ...
        function dlSpectrogram(obj, timeW, freqW, overlap, fmax, xlims, clims)
            ...
            for ik = 1:size(X, 1)
                ...
                Xt = detrend(Xt);
                Xt = smooth(Xt, 10);
                [sGt, ~, ~] = pspectrum(Xt, 1000, "spectrogram", "FrequencyLimits", [0 fmax], "TimeResolution", 0.5, "OverlapPercent", 98);
                sG = sG + sGt;
            ...
            pre = floor(size(sG, 2)*0.16):floor(size(sG, 2)*0.35);
            post = floor(size(sG, 2)*0.45):floor(size(sG, 2)*0.6);

            preSpec = mean(sG(10:end, pre), 2);
            postSpec = mean(sG(10:end, post), 2);
            preDom = find(preSpec == max(preSpec));
            postDom = find(postSpec == max(postSpec));

            % beta = floor(size(sG, 1)*0.15):floor(size(sG, 1)*0.3);
            % gamma = floor(size(sG, 1)*0.32):floor(size(sG, 1)*0.5);

            rG1 = [preDom, postDom]*fmax/size(sG, 1);

            subplot(2, 1, 2);
            bar(["pre-stim", "during-stim"], rG1);title("Dominant frequency");
            ylabel("Frequency(Hz)");
            % ylim([0 0.2]);

            sgtitle("Spectrogram / Dominant frequency");
