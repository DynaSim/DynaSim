function [pacmat, freqvec_ph, freqvec_amp] = find_pac_shf_fdr (sig_pac, Fs, measure, ...
...
if num_shf ~= 0
    ...
    for i = 1:size(pacmat,1)
        for j = 1:size(pacmat,2)
            [h(i,j), p(i,j)] = my_sig_test_fdr(pacmat(i,j), squeeze(shf_pacmat_final(:,i,j)),...
                alpha, measure);            
%             if h == 0
%                 pacmat(i,j) = 0;
%             end
    ...
    p_array = p(:);
    p_array = sort(p_array,'ascend');
    for i = 1:length(p_array)
        new_alpha = ((i/length(p_array))*alpha);
        if p_array(i) <= ((i/length(p_array))*alpha)
            break
        end
    end
