...
% % Possible inputs:
% 1 sim, 1 pop, 1 vars, N cells
% 1 sim, X pop, 1 vars, N cells
% 1 sim, 1 pop, X vars, N cells
% 1 sim, X pops, X vars, N cells
% X sims, 1 pop, 1 vars, N cells
% X sims, x pop, 1 vars, N cells
% X sims, 1 pop, x vars, N cells
%     Don't average across cells; separate large figures

% in general, vars go on separate figures
% N cells go on overlap
% if have 1 cell, can then do multiple pops on overlay
% if have 1 cell and 1 pop, can do multiple sims on overlay
...
if num_sims>1 && isfield(data,'varied')
  ...
  for j=1:num_varied
    if isnumeric(data(1).(varied{j}))
      ...
      % (eg, mechanisms) (also in PlotFR, SelectData, and DynaSim2CellArray)
    end
  end
...
function vars_out = getdefaultvar(xp)
    ...
    vars = xp.axis(xp.findaxis('variables')).values;
    
    possibilities = {'V','X','Vm','Xm','Y','Ym'};
    ind = []
    i=0;
