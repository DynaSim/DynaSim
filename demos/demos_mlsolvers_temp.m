...
data=SimulateModel(eqns,'tspan',[0 100],'study_dir','lorenz_ml_solver', 'ic',[1 2 .5],'solver','ode45',...
  'verbose_flag',1);
PlotData(data)
...
data=SimulateModel(eqns,'dt',0.01, 'downsample_factor',100, 'study_dir','hh_ml_solver', 'solver','ode45',...
  'verbose_flag',1);
...
% data=SimulateModel('dv/dt=10+@current/Cm; Cm=1; v(0)=-65; {iNa,iK}','study_dir','hh_predef_ml_solver', 'solver','ode45');
% figure; plot(data.time,data.(data.labels{1}))
% xlabel('time (ms)'); ylabel('membrane potential (mV)'); title('Hodgkin-Huxley neuron')
% 
...
% data=SimulateModel(eqns,'tspan',[0 200],'study_dir','hh_active_ml_solver', 'solver','ode45');
% figure; plot(data.time,data.(data.labels{1}))
...
data=SimulateModel(s,'dt',0.01, 'downsample_factor',100, 'study_dir','sPing_ml_solver',...
...
% od45
SimulateModel(eqns,'save_data_flag',1,'study_dir','demo_cluster_1_ml_solver_ode45',...
...
% ode45 compiled
SimulateModel(eqns,'save_data_flag',1,'study_dir','demo_cluster_1_ml_solver_ode45_compiled',...
                   'vary',vary,'cluster_flag',1,'overwrite_flag',1,'verbose_flag',1,...
                   'dt',0.01, 'downsample_factor',100,'solver','ode45',...
                    'matlab_solver_options', {'InitialStep', 0.01}, 'compile_flag',1);
               
% ode23
SimulateModel(eqns,'save_data_flag',1,'study_dir','demo_cluster_1_ml_solver_ode23s',...
                   'vary',vary,'cluster_flag',1,'overwrite_flag',1,'verbose_flag',1,...
                   'dt',0.01, 'downsample_factor',100,'solver','ode23s',...
                    'matlab_solver_options', {'InitialStep', 0.01}, 'compile_flag',0);
                  
% ode23 compiled
SimulateModel(eqns,'save_data_flag',1,'study_dir','demo_cluster_1_ml_solver_ode23s_compiled',...
                   'vary',vary,'cluster_flag',1,'overwrite_flag',1,'verbose_flag',1,...
                   'dt',0.01, 'downsample_factor',100,'solver','ode23s',...
                    'matlab_solver_options', {'InitialStep', 0.01}, 'compile_flag',1);
