...
clear;clc;
cd('//Users/hamednejat/Works/Research/dsm');
...
id = 1;
m = DynaLearn(); % ~ 1sec
m = m.dlLoad(char("dlModels/dlRhythmModels" + string(id))); % ~ 10sec, New larger model; keeping track of its activity in Gamma/Beta **
...
clc;
m = DynaLearn(); % ~ 1sec
m = m.dlLoad(char("dlModels/dlRhythmModels" + string(1)));
...
TotalSize = ones(1, 10)*50;
noise_rate = 4.7;

for model_size_id = 1:1

    Currentsize = TotalSize(model_size_id);
    m = dlRhythmModel(1, Currentsize, model_size_id, noise_rate);
