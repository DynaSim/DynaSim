function m = dlTCTPerformerTripple(Currentsize, model_size_id, noise_rate)

    %%% Create model parameters struct
    ModelParametersPFC = struct();
    PFCSize = Currentsize;
    V4ASize = ceil(Currentsize*0.2);

    %%% Area PFC layer sizes (relative)
    ModelParametersPFC.NeSuperficial = ceil(0.3*PFCSize);
    ModelParametersPFC.NSomSuperficial = ceil(0.02*PFCSize);
    ModelParametersPFC.NPvSuperficial = ceil(0.04*PFCSize);
    ModelParametersPFC.NeMid = ceil(0.14*PFCSize);
    ...
    ModelParametersV4.NeSuperficial = ceil(0.25*V4ASize);
    ModelParametersV4.NSomSuperficial = ceil(0.03*V4ASize);
    ModelParametersV4.NPvSuperficial = ceil(0.07*V4ASize);
    ModelParametersV4.NeMid = ceil(0.12*V4ASize);
    ModelParametersV4.NSomMid = 0;
    ModelParametersV4.NPvMid = ceil(0.03*V4ASize);
    ModelParametersV4.NeDeep = ceil(0.45*V4ASize);
    ModelParametersV4.NSomDeep = ceil(0.03*V4ASize);
    ModelParametersV4.NPvDeep = ceil(0.02*V4ASize);
    ...
    targetParams4 = [{'EPenalty', 4:9, 200, 0.01}; {'Compare', [3, 1], 0, 2}; {'Compare', [3, 2], 0, 2}; {'Compare', [2, 3], 0, 2}; {'Compare', [1, 3], 0, 2}]; % Null
