...
%
% See also: ds.checkSpecification, ds.checkModel, ds.parseModelEquations, ds.simulateModel
...
if ~isempty(options.modifications)
  specification=ds.applyModifications(specification,options.modifications);
end
...
for i=1:npops
  ...
  if ~isempty(specification.populations(i).model)
    ...
    if ~strcmp(specification.populations(i).name,tmpname)
      ...
      tmpmodel=ds.applyModifications(tmpmodel,{tmpname,'name',specification.populations(i).name});
    elseif strcmp(tmpname,'pop1') % if default name
      % use default name for this population index
      tmpmodel=ds.applyModifications(tmpmodel,{tmpname,'name',sprintf('pop%g',i)});
    end
...
if ~isempty(model.monitors)
  ...
  function add_keywords(src,dst,namespace)
    % NOTE: this needs to be coordinated with update_keywords() in ds.simulateModel()
    %   for parameters
    Nsrc=[src '_Npop'];
    Ndst=[dst '_Npop'];
