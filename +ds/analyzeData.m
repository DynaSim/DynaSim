...
%
% Usage:
%   result = ds.analyzeData(data,func,'option1',value1,...)
%
% Inputs:
...
if numel(func)>1
  error('Too many function handles were supplied. ds.analyzeData only applies a single function to a single data set.');
end
...
% example, ds.plotData2 returns a nested structure of figure, axis, and plot
...
if all(ishandle(result)) % analysis function returned a graphics handle
  ...
  function result = add_modifications(result)
    ...
    % prepare_varied_metadata in ds.simulateModel, the data structure contains
