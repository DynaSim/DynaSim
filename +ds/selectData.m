...
%
% See also: ds.simulateModel, ds.vary2Modifications, ds.importData
...
if ~isempty(options.varied) && isfield(data,'varied')
  if size(options.varied,2)==3
    % convert into field name (set in ds.simulateModel:prepare_varied_metadata)
    % note: this code is redundant with ds.simulateModel and ds.analyzeData.
  ...
  if size(options.varied,1)>1
    error('at present, a range on only one varied parameter can be specified per call to ds.selectData.');
  end
  ...
  for j=1:num_varied
    if isnumeric(data(1).(varied{j}))
      ...
      % (eg, mechanisms) (also in ds.plotData)
    end
  end
...
% % recursively call ds.selectData if more than one data set
% if length(data)>1
%   for i=1:length(data)
%     data(i)=ds.selectData(data(i),varargin{:});
%   end
%   return;
% end
