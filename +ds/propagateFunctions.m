...
%
% See also: ds.simulateModel, ds.generateModel, ds.propagateNamespaces
...
while keep_going
  ...
  for i=1:length(expressions)
    ...
    [expressions{i},keep_going]=insert_functions(expressions{i},functions);
...
%% SUBFUNCTIONS
function [expression,functions_were_found]=insert_functions(expression,functions)
...
%       new_var_list=regexp(expression,[found_function '\(*\(([^\)\(]+)\)'],'tokens','once');
      ...
      if ~isequal(orig_vars,new_vars)
        for v=1:length(orig_vars)
          found_expression=ds.strrep(found_expression,orig_vars{v},new_vars{v});
        end
      end
      ...
      expression=ds.strrep(expression,oldstr,newstr,'(',')');
    end
  end
