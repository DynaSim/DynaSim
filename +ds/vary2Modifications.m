...
% ds.applyModifications to produce the modified object.
%
% Usage:
...
%     data(i)=ds.simulateModel('dv/dt=@current+10; {iNa,iK}','modifications',mod_set{i});
%     figure; plot(data(i).time,data(i).(data(i).labels{1}))
%   end
%   % note: the same data set can be obtained directly from ds.simulateModel by:
%   data=ds.simulateModel('dv/dt=@current+10; {iNa,iK}','vary',vary);
%
%   vary={'E','gNa',[100 120]};
...
%   modifications_set{:}
%
% See also: ds.applyModifications, ds.simulateModel, ds.generateModel
