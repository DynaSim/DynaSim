...
%
% See also: ds.simulateModel, ds.updateStudy

% Check inputs
opts=ds.checkOptions(varargin,{...
  'modifications_set',[],[],... % search space
  'simulator_options',[],[],... % options from ds.simulateModel
  'process_id',[],[],... % process identifier for loading studyinfo if necessary
  },false);

if isempty(opts.simulator_options)
  error('ds.setupStudy must be given a simulator_options structure from ds.simulateModel');
end
